...
**Routing function for judgement calls:** `criticality = frequency it occurs × blast radius when it does`. Whenever a case doesn't route cleanly by persona (tier-3 cases especially), compute this, not a gut label. High criticality → automated blocking test at the appropriate level; low → `Manual-QA:` line. Both factors count: a daily cosmetic bug hitting 20% of orders out-ranks a scary-looking input that never occurs organically.
...
| 3. Long-tail | Only a CSE grad poking at it: obscure paths, adversarial inputs — "what if I enter 0 / -1? birth date in the future?" | High-criticality cases (per the equation): unit tests (blocking). Rest: manual QA, marked in the MR | Unit part blocks |

## Adversarial inputs are a dimension, not just tier 3

Input edge cases exist on every path — grandma typos her phone number on the critical path too. Apply the tier-3 litmus wherever input enters, on any tier. Route each case by the criticality equation: high → blocking unit test; rest → marked manual QA.

## Rules

1. **Impact override.** A blast radius large enough (money movement, data destruction, security breach) clears the criticality bar even at frequency ≈ 0 — automated blocking coverage regardless of persona. Admin refund tool = CSE-grad traffic, tier-1 impact.
...
- Adversarial: address change racing the shipped transition → wrong delivery, costs money → huge blast radius, criticality high at any frequency → blocking test. Out-of-zone address, double-click double-submit → low frequency × recoverable harm → `Manual-QA:` lines in the MR.

## Common mistakes

- Cutting by intuition instead of running the litmus tests — the rationale becomes unfalsifiable.
- Routing edge cases by how scary they look instead of frequency × blast radius — over-tests theatrical never-occurring inputs, under-tests mundane daily breakage.
